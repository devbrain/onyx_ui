    ...
    class themeable {
        public:
            ...
            [[nodiscard]] virtual resolved_style<Backend> resolve_style(
                ...
                // Defensive: Handle null theme gracefully by using parent style
                // NOTE: Public APIs should enforce non-null themes, but this provides
                // a safety net to avoid crashes if called incorrectly
                if (!theme) {
                    resolved_style<Backend> style = parent_style;
                    // Still apply explicit overrides (highest priority)
                    if (m_background_override) style.background_color = *m_background_override;
                    if (m_foreground_override) style.foreground_color = *m_foreground_override;
                    if (m_box_style_override) style.box_style = *m_box_style_override;
                    if (m_font_override) style.font = *m_font_override;
