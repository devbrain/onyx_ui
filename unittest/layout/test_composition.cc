...
TEST_SUITE("layout_composition") {
    ...
    TEST_CASE("Vertical with horizontal rows") {
        ...
        CHECK(measured.width == 250);  // 3*80 + 2*5 = 250
        CHECK(measured.height == 70);  // 30 + 10 + 30 = 70
        ...
        CHECK(r1->bounds().x == 0);
        CHECK(r1->bounds().y == 0);
        CHECK(r1->bounds().height == 30);

        // Verify row 1's children
        CHECK(r1->child_at(0)->bounds().x == 0);
        CHECK(r1->child_at(0)->bounds().width == 80);
        ...
        CHECK(r2->bounds().height == 30);

        // Verify row 2's expanding children
        CHECK(r2->child_at(0)->bounds().width == 148);  // (300-5)/2, first gets extra
        CHECK(r2->child_at(1)->bounds().width == 147);
    }
    ...
    TEST_CASE("Horizontal with vertical columns") {
        ...
        CHECK(measured.width == 165);  // 150 + 15 (col2 has no measured width yet)
        CHECK(measured.height == 130);  // Max of columns: 3*40 + 2*5
        ...
        CHECK(c2->bounds().width == 235);  // 400 - 150 - 15
    ...
    TEST_CASE("Vertical with grid sections") {
        ...
        CHECK(g->child_at(0)->bounds().height == 100);
        CHECK(g->child_at(1)->bounds().height == 100);
        CHECK(g->child_at(2)->bounds().height == 100);
        CHECK(g->child_at(3)->bounds().height == 100);
    ...
    TEST_CASE("Grid with vertical form fields") {
        ...
        CHECK(measured.width >= 160);  // At least 2 columns worth
        CHECK(measured.height == 55);  // 20 + 5 + 30
    ...
    TEST_CASE("Anchor with linear toolbar and content") {
        ...
        CHECK(sb->bounds().width == 100);

        // Verify sidebar's items
        CHECK(sb->child_at(0)->bounds().height == 35);
        CHECK(sb->child_at(1)->bounds().height == 35);
        CHECK(sb->child_at(2)->bounds().height == 35);
    }
    ...
    TEST_CASE("Absolute with linear child panels") {
        ...
        CHECK(p1->bounds().width == 100);  // Explicit width
        CHECK(p1->bounds().height == 65);  // 2*30 + 5 spacing
        ...
        CHECK(p2->bounds().width == 130);  // 3*40 + 2*5
        CHECK(p2->bounds().height == 25);
    ...
    TEST_CASE("Grid with different child layouts") {
        ...
        CHECK(vertical_cell->child_at(0)->bounds().height == 25);
        CHECK(vertical_cell->child_at(1)->bounds().height == 25);

        auto horizontal_cell = grid->child_at(1);
        CHECK(horizontal_cell->child_count() == 2);
        // Both should expand to fill cell
        CHECK(horizontal_cell->child_at(0)->bounds().width > 0);
        CHECK(horizontal_cell->child_at(1)->bounds().width > 0);
