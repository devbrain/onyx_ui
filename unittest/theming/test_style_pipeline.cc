...
namespace {
    ...
    ui_theme<Backend> create_pipeline_test_theme() {
        ...
        theme.button.hover.background = {0, 140, 235};
        theme.button.pressed.foreground = {200, 200, 200};
        theme.button.pressed.background = {0, 100, 195};
        theme.button.disabled.foreground = {150, 150, 150};
        theme.button.disabled.background = {80, 80, 80};
    ...
    ui_theme<Backend> create_alternate_theme() {
        ...
        theme.button.normal.foreground = {0, 0, 0};
        theme.button.normal.background = {225, 225, 225};
        theme.button.hover.foreground = {0, 0, 0};
        theme.button.hover.background = {200, 200, 200};
        theme.button.pressed.foreground = {0, 0, 0};
        theme.button.pressed.background = {180, 180, 180};
        theme.button.disabled.foreground = {120, 120, 120};
        theme.button.disabled.background = {240, 240, 240};
...
//     CHECK(success == true);  // apply_theme() removed
    ...
    CHECK(bg.r == 0);   // button.normal.background
    CHECK(bg.g == 120);
    CHECK(bg.b == 215);

    auto fg = btn->resolve_style().foreground_color;
    CHECK(fg.r == 255);  // button.normal.foreground
    CHECK(fg.g == 255);
    CHECK(fg.b == 255);
...
//     btn->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    auto bg = btn->resolve_style().background_color;
    CHECK(bg.g == 120);  // button.normal.background
}
...
//     root->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    auto* panel4 = panel3->template emplace_child<panel>();

    // Layout entire tree
    [[maybe_unused]] auto _ = root->measure(300, 300);
    root->arrange({0, 0, 300, 300});

    // Panel at depth 5 should still have theme colors (testing CSS inheritance)
    auto fg = panel4->resolve_style().foreground_color;
    CHECK(fg.r == 240);
    CHECK(fg.g == 240);
    CHECK(fg.b == 240);
}
...
//     root->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    auto fg = child->resolve_style().foreground_color;
    CHECK(fg.r == 200);
    CHECK(fg.g == 0);
    CHECK(fg.b == 0);
}
...
//     parent->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    auto fg1 = child->resolve_style().foreground_color;
    CHECK(fg1.r == 100);
    ...
    auto fg2 = child->resolve_style().foreground_color;
...
//     root->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    // Initial colors (dark theme) - btn1 uses button theme colors
    auto bg1_dark = btn1->resolve_style().background_color;
...
//     root->apply_theme("Alternate", ctx.themes());  // No longer needed - widgets use global theme
    ...
    // Colors should update - buttons use button-specific theme colors
    auto bg1_light = btn1->resolve_style().background_color;
    auto bg2_light = btn2->resolve_style().background_color;

    CHECK(bg1_light.r == 225);  // Alternate theme: button.normal.background = {225, 225, 225}
    CHECK(bg2_light.r == 225);
}
...
//     root->apply_theme("Alternate", ctx.themes());  // No longer needed - widgets use global theme
    ...
    // Background should update from theme - buttons use button-specific colors
    auto bg = btn->resolve_style().background_color;
    CHECK(bg.r == 225);  // Alternate theme: button.normal.background = {225, 225, 225}
}
...
//     middle->apply_theme("Alternate", ctx.themes());  // Different theme!  // No longer needed - widgets use global theme
    ...
    // Button uses its own theme colors (middle has Alternate theme, root has Pipeline Test)
    auto bg = btn->resolve_style().background_color;
    CHECK(bg.r == 240);  // middle's Alternate theme: panel.background = {240, 240, 240}
}
...
//     outer_box->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    // Both buttons should have button-specific theme colors
    auto bg1 = btn1->resolve_style().background_color;
    auto bg2 = btn2->resolve_style().background_color;

    CHECK(bg1.g == 120);  // button.normal.background = {0, 120, 215}
    CHECK(bg2.g == 120);
}
...
//     root->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    auto panel_bg = pnl->resolve_style().background_color;
    auto label_fg = lbl->resolve_style().foreground_color;
    auto button_bg = btn->resolve_style().background_color;

    CHECK(panel_bg.r == 10);   // Inherits window_bg
    CHECK(label_fg.r == 240);  // Inherits text_fg
    CHECK(button_bg.g == 120); // button.normal.background = {0, 120, 215}
}
...
//     root->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    // New button should inherit theme - both use button-specific colors
    auto bg1 = btn1->resolve_style().background_color;
    auto bg2 = btn2->resolve_style().background_color;

    CHECK(bg1.g == bg2.g);  // Both have theme colors
    CHECK(bg2.g == 120);  // button.normal.background = {0, 120, 215}
}
...
//     root->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    for (auto const& child : children) {
        if (auto* btn = dynamic_cast<button<Backend>*>(child.get())) {
            auto bg = btn->resolve_style().background_color;
            CHECK(bg.g == 120);  // button.normal.background = {0, 120, 215}
        }
    }
}
...
//             btn->apply_theme("Alternate", ctx.themes());  // No longer needed - widgets use global theme
    ...
    // Final theme should be Alternate (button uses button.normal.background)
    auto bg = btn->resolve_style().background_color;
    CHECK(bg.r == 225);  // Alternate theme: button.normal.background = {225, 225, 225}
}
...
//     root->apply_theme("Pipeline Test", ctx.themes());  // No longer needed - widgets use global theme
    ...
    // Button inherits foreground from p1, background from p2
    auto fg = btn->resolve_style().foreground_color;
    auto bg = btn->resolve_style().background_color;

    CHECK(fg.r == 200);  // From p1 override (foreground inherits)
    CHECK(bg.g == 200);  // From p2 override (background inherits)
}
